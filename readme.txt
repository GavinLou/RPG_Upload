...
<html lang="en-us">
  <head>
    ...
    <title>Unity WebGL Player | RPG_DEMO</title>
    <link rel="shortcut icon" href="TemplateData/favicon.ico">
    <link rel="stylesheet" href="TemplateData/style.css">
  </head>
  <body>
      <div id="unity-container" class="unity-desktop" style="width: 100%; height: auto; aspect-ratio: 9 / 16; overflow: hidden; ">
      ...
      <script>  
          ...
          async function notifyUnityFirebaseReady() {
              try {
                  ...
                  // 確保 Unity 物件已經初始化並準備好s
                  if (window.unityInstance) {
                      setTimeout(() => {
                          console.log("Firebase initialize wait！");
                          window.unityInstance.SendMessage("GameManager", "OnFirebaseInitialized", "Firebase is ready");
                      }, 1000); // 延遲 1 秒再發送訊息，確保 Unity 加載完成
                  } else {
                      console.error("Unity 尚未初始化，無法發送 Firebase 完成訊息！");
                  }
              } catch (error) {
